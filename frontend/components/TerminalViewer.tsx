...
export default function TerminalViewer({
    ...
    const connectWebSocket = () => {
        ...
        ws.onmessage = (event) => {
            try {
                ...
                // NEW: Handle referenced files update
                if (thoughtEvent.data?.referenced_files && Array.isArray(thoughtEvent.data.referenced_files)) {
                    setReferencedFiles(prev => {
                        const newFiles = thoughtEvent.data.referenced_files as string[];
                        // Combine and deduplicate
                        const combined = [...new Set([...prev, ...newFiles])];
                        return combined;
                    });
                }
            } catch (error) {
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex flex-col bg-black/40 border border-[#2a3a27] rounded-xl overflow-hidden backdrop-blur-sm h-[calc(100vh-18rem)]">
                {/* Terminal Header */}
            ...
            {/* NEW: Context Map integration */}
            <div className="h-48">
                <ContextMap referencedFiles={referencedFiles} isProcessing={isProcessing} />
            </div>
